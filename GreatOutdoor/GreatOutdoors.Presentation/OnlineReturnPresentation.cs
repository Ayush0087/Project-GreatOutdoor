                                ...
                                if (confirmation.Equals("Y", StringComparison.OrdinalIgnoreCase))
                                {
                                    using (IOnlineReturnBL onlineReturnBL = new OnlineReturnBL())
                                    {
                                        ...
                                        if (confirmed)
                                        {
                                            ...
                                            matchingOrder[ProductToBeReturned - 1].ReturnAmount -= matchingOrder[ProductToBeReturned - 1].PrductPrice * quantityToBeReturned;
                                            Console.WriteLine("Total Return Amount: " + (matchingOrder[ProductToBeReturned - 1].ProductPrice * quantityToBeReturned));
                                            OrderDetailBL orderDetailBL1 = new OrderDetailBL();
                                            await orderDetailBL1.UpdateOnlineReturnBL(matchingOrder[ProductToBeReturned - 1]);
